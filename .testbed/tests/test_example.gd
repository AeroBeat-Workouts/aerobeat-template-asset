...
const EXPECTED_PLUGIN_DESCRIPTION := "Template for AeroBeat internal asset repos. Shared internal/runtime assets built on aerobeat-asset-core for current v1 work."
...
func test_readme_keeps_v1_internal_asset_template_truth() -> void:
	var readme_text := _read_repo_file(README_PATH)
	assert_true(readme_text.contains("official template for creating **internal asset** repositories"), "README should state that this repo is an internal asset template")
	...
	assert_true(readme_text.contains("internal/system asset work"), "README should preserve the downscoped internal-only asset stance")
	assert_true(readme_text.contains("package-local gameplay asset swap story"), "README should explicitly reject the old package-local asset swap framing")
...
func test_plugin_cfg_description_stays_template_specific() -> void:
	...
	assert_eq(config.get_value("plugin", "name", ""), "AeroBeat Internal Asset Template", "plugin.cfg name should stay stable")
	assert_eq(
		config.get_value("plugin", "description", ""),
		EXPECTED_PLUGIN_DESCRIPTION,
		"plugin.cfg description should remain aligned with the template's narrow v1 asset-lane contract"
	)
